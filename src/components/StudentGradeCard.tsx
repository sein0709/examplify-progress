...
export const StudentGradeCard = ({
  ...
  const fetchQuickStats = async () => {
    try {
      const {
        data: submissions,
        error
      } = await supabase.from("submissions").select("score, total_questions, submitted_at").eq("student_id", studentId).order("submitted_at", {
        ascending: false
      });
      if (error) throw error;
      const gradedSubmissions = submissions?.filter(s => s.score !== null) || [];
      const scores = gradedSubmissions.map(s => Math.round(s.score! / s.total_questions * 100));
      ...
      if (scores.length >= 4) {
        ...
        if (recent > older + 5) trend = "up";else if (recent < older - 5) trend = "down";
      }
      setStats({
        averageScore,
  ...
  return <StudentScoreDialog studentId={studentId} studentName={studentName} trigger={<Card className={cn("group relative overflow-hidden cursor-pointer", "border border-border/50 hover:border-primary/30", "bg-gradient-to-br from-card to-card/80", "hover:shadow-lg hover:shadow-primary/5", "transition-all duration-300 ease-out", "hover:-translate-y-1", "animate-fade-in")} style={{
    animationDelay: `${index * 50}ms`
          ...
          <div className="relative p-4">
            <div className="flex items-center gap-4">
              {/* Avatar */}
              <div className={cn("relative flex-shrink-0 w-12 h-12 rounded-full", "bg-gradient-to-br", getAvatarGradient(studentName), "flex items-center justify-center", "text-white font-semibold text-sm", "shadow-md group-hover:shadow-lg", "transition-all duration-300", "group-hover:scale-105")}>
                ...
                <div className="absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full bg-card flex items-center justify-center">
                  <div className={cn("w-2 h-2 rounded-full", stats?.submissionCount && stats.submissionCount > 0 ? "bg-emerald-500" : "bg-muted-foreground/30")} />
                </div>
              </div>

              {/* Info */}
              <div className="flex-1 min-w-0">
                <h3 className="font-semibold text-foreground truncate group-hover:text-accent transition-colors duration-200">
                  {studentName}
                </h3>
                
                {loading ? <div className="h-4 w-24 bg-muted animate-pulse rounded mt-1" /> : stats ? <div className="flex items-center gap-2 mt-1">
              ...
              <div className="flex items-center gap-3">
                {!loading && stats && stats.submissionCount > 0 && <Badge variant="secondary" className="hidden sm:flex bg-muted/50 text-muted-foreground font-normal">
